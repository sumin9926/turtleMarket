...
public class KakaoMessageService {
    ...
    public void sendInTransitMessage(UserNotification userNotification) {
        String templateObject = kakaoMessageBuilder.buildTextInTransitTemplateObject(userNotification);

        if (templateObject == null || templateObject.isBlank()) {
            log.warn("⚠️ 배송 중 템플릿 생성 실패로 메시지 전송이 중단되었습니다.");
            return;
        }
    ...
    public void sendDeliveredMessage(UserNotification userNotification) {
        String templateObject = kakaoMessageBuilder.buildTextDeliveredTemplateObject(userNotification);

        if (templateObject == null || templateObject.isBlank()) {
            log.warn("⚠️ 배송 완료 템플릿 생성 실패로 메시지 전송이 중단되었습니다.");
            return;
        }
